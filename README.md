...
Десктопное приложение запускает Telegram-бота на базе aiogram и направляет текст и изображения в уже работающий OpenAI-совместимый llama-server. Настройки редактируются в нативном интерфейсе tkinter; кнопки Start и Stop управляют ботом без остановки GUI.
...
- фотография или изображение-документ до 10 МиБ — отправить изображение vision-модели. Подпись используется как вопрос, а без подписи бот попросит модель описать изображение.

При обращении к модели бот сразу отправляет сообщение `...`, получает ответ от llama-server в потоковом режиме и редактирует это же сообщение после каждых 150 сгенерированных токенов. После завершения генерации в нём отображается полный ответ; дополнительные сообщения для частей ответа не создаются.

## Поддержка изображений

Бот принимает обычные фотографии Telegram и изображения, отправленные как документы с MIME-типом `image/*`. Максимальный размер одного изображения — 10 МиБ. Подпись к изображению передаётся модели как запрос; если подписи нет, используется запрос «Опиши изображение.».

Изображения отправляются только модели, для которой llama-server сообщает модальность `vision` через `/props`. Если загружена обычная текстовая модель, бот отклонит изображение и сообщит, что анализ изображений недоступен.

История хранится отдельно для каждого чата только в памяти и очищается после Stop. Как и в llama-server UI, изображение остаётся частью сообщения и повторно передаётся модели при следующих вопросах. `/reset` удаляет его вместе с историей; старые пары «пользователь/ассистент» с их изображениями также удаляются, когда история приближается к `n_ctx`. Другие типы вложений модели не отправляются.
...
- **Модель не анализирует изображения:** убедитесь, что `/props` сообщает модальность `vision`; обычная текстовая модель не получит вложение.
- **Изображение отклонено:** отправьте фотографию или документ с MIME-типом `image/*` размером не более 10 МиБ.
